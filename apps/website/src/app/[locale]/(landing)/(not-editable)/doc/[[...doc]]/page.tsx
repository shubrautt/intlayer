...
const DocumentationPage = async ({
  ...
  const contentUrl = docData.githubUrl
    .replace('/en/', `/${locale}/`)
    .replace(
      'github.com/aymericzip/intlayer/blob',
      'raw.githubusercontent.com/aymericzip/intlayer/refs/heads'
    );

  const docContent = await fetch(contentUrl).then((res) => res.text());
